...
export default class index extends Component {
  ...
  renderMenusTable(){
   ...
   const handleAddMenus = (record) => {
    ...
    return (
    <Table pagination={false} className="menus-table" dataSource = {dataSource} columns={columns} />
    );
  }
  ...
  renderCartTable(){
      const columns = [
        {
          ...
          render:(text,record) =>(
              <span>
                  <Button onClick={() => handleDecrease(record)} className={style['cart-btn']}>-</Button>
                  <span>{record.count}</span>
                  <Button onClick={() => handleIncrease(record)} className={style['cart-btn']}>+</Button>
              </span>
          )
         },
         {
    ...
    return (
        <Table pagination={false} className="menus-table cart" dataSource = {this.state.cart} 
               columns={columns} locale={{emptyText:"购物车没有任何商品"}} />
        );
  }
